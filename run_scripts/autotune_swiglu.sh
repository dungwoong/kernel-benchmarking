...
# sbatch --cpus-per-task=8 --mem=64G --time=03:00:00  --export=ALL,CONTAINER="kernel_bench_apptainer.sif",SCRIPT="run_scripts/autotune_swiglu.sh" slurm_profile.sh

DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

CSV="${1:-results/helion_autotune_swiglu_$(date +%Y%m%d_%H%M%S).csv}"
...
for i in $CONFIGS; do
    echo "=== autotuning helion swiglu config $i ==="
    python3 "$DIR/demo/helion/swiglu_autotune.py" "$i" --csv "$CSV"
done
