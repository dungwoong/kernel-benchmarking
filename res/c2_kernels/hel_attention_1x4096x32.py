...
class Kernel:
  ...
  def __call__(self, mQ: cute.Tensor, mK: cute.Tensor, mV: cute.Tensor, mO: cute.Tensor):
    ...
    st_5635_layout = shared.get_smem_layout_row_major(cutlass.BFloat16, 128, 128, 2)
    ...
    mV_tma_atom_4, mV_tma_tensor_4 = shared.get_tma_tensor_and_atom(mV, st_5635_layout, 128, 128, 1)
    self.kernel(mQ, mK, mV, st_5538_layout, st_5556_layout, st_5635_layout, mO, rt_5665_epi_smem_layout, tiled_mma_5524, tiled_mma_5605, wgmma_acc_5651_tiled_mma, tiled_mma_6327, tiled_mma_6374, tiled_mma_6392, mO_tma_atom_1, mO_tma_tensor_1, mQ_tma_atom_2, mQ_tma_tensor_2, mK_tma_atom_3, mK_tma_tensor_3, mV_tma_atom_4, mV_tma_tensor_4).launch(grid=[132, 1, 1], block=256, cluster=[1, 1, 1])

  @cute.kernel
  def kernel(self, mQ: cute.Tensor, mK: cute.Tensor, mV: cute.Tensor, st_5538_layout, st_5556_layout, st_5635_layout, mO: cute.Tensor, rt_5665_epi_smem_layout, tiled_mma_5524, tiled_mma_5605, wgmma_acc_5651_tiled_mma, tiled_mma_6327, tiled_mma_6374, tiled_mma_6392, mO_tma_atom_1, mO_tma_tensor_1, mQ_tma_atom_2, mQ_tma_tensor_2, mK_tma_atom_3, mK_tma_tensor_3, mV_tma_atom_4, mV_tma_tensor_4):
    ...
    shared.smem_add_shared_tensor(SharedStorage_t, 'st_5635_ptr', cutlass.BFloat16, st_5635_layout, 1024)
    shared.smem_add_barrier_array(SharedStorage_t, 'st_5635_pipe_ptr', 2)
    ...
    st_5635 = shared.smem_get_tensor(smem_, 'st_5635_ptr', st_5635_layout)
    st_5635_pipe = pipeline.make_tma_pipeline_alt(smem_, 'st_5635_pipe_ptr', 2, shared.staged_tensor_sizes(cutlass.BFloat16, st_5635_layout), 4, cute.make_layout((1, 1, 1, 1)), 1)
    ...
    st_5635_pstate = cutlass.pipeline.make_pipeline_state(cutlass.pipeline.PipelineUserType.Producer, 2)
    st_5635_cstate = cutlass.pipeline.make_pipeline_state(cutlass.pipeline.PipelineUserType.Consumer, 2)
    if warpidx_ >= 0 and warpidx_ < 4:
      ...
      for sched_idx in cutlass.range(cute.arch.block_idx()[0], 32, 132):
        ...
        gt_slice_5627 = mV[None, None, sched_coord[0]]
        gt_slice_5530 = mQ[None, None, sched_coord[0]]
        gt_slice_5643 = mO[None, None, sched_coord[0]]
        st_5538_pipe.consumer_wait(st_5538_cstate, st_5538_pipe.consumer_try_wait(st_5538_cstate))
        for k in cutlass.range(0, 32, 1):
          rt_5526 = cute.make_rmem_tensor_like(rt_5523, cutlass.Float32)
          rt_5526.store(elementwise.copy_elementwise(rt_5523.load()))
          ...
          reduction.row_max_f32(rt_5569, rt_5523)
          rt_5523.store(reduction.warp_max_row_mma_layout(rt_5523.load()))
          rt_5581 = elementwise.row_bcast_sub(rt_5569, rt_5523)
          rt_5587 = cute.make_rmem_tensor_like(rt_5581, cutlass.Float32)
          rt_5587.store(elementwise.exp2f(rt_5581.load()))
          rt_5587_frgA = cute.make_tensor(rt_5587.iterator, layout.convert_layout_acc_frgA(rt_5587.layout))
          rt_5594 = elementwise.tilewise_sub(rt_5526, rt_5523)
          rt_5600 = cute.make_rmem_tensor_like(rt_5594, cutlass.Float32)
          rt_5600.store(elementwise.exp2f(rt_5594.load()))
          rt_5604.store(elementwise.tilewise_mul(rt_5604, rt_5600).load())
          wgmma_acc_5651.store(elementwise.row_mul(wgmma_acc_5651, rt_5600).load())
          reduction.row_sum_mixed_types(rt_5587_frgA, rt_5604, cutlass.Float32)
          rt_6888 = conversion.cvt_f16(rt_5587_frgA, cutlass.BFloat16)
          st_5556_pstate.advance()
          st_5556_cstate.advance()
          st_5635_pipe.consumer_wait(st_5635_cstate, st_5635_pipe.consumer_try_wait(st_5635_cstate))
          mma.accumulating_gemm_rs(tidx_, tiled_mma_6374, rt_6888, layout.transpose_view(st_5635), wgmma_acc_5651, st_5635_cstate, wgmma_acc_5651_accumulate, -1)
          wgmma_acc_5651_accumulate = True
          cute.nvgpu.warpgroup.wait_group(0)
          st_5635_pipe.consumer_release(st_5635_cstate)
          st_5635_pstate.advance()
          st_5635_cstate.advance()
        ...
        rt_5665 = elementwise.row_mul(wgmma_acc_5651, rt_5623)
      ...
      if warpidx_ == 4:
        ...
        for sched_idx in cutlass.range(cute.arch.block_idx()[0], 32, 132):
          ...
          for k in cutlass.range(0, 32, 1):
            ...
            st_5556_pstate.advance()
            if cutlass.const_expr(True):
              st_5635_pipe.producer_acquire(st_5635_pstate, st_5635_pipe.producer_try_acquire(st_5635_pstate))
              ...
              shared.tma_copy(mV_tma_atom_4, mV_tma_tensor_4_slice, st_5635, 128, 128, k, 0, st_5635_pipe, st_5635_pstate, cta_coord_4, cta_layout_4, mcast_mask_4)
            st_5635_pstate.advance()
          st_5538_pstate.advance()
