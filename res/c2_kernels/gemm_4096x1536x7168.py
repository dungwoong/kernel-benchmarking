...
class Kernel:
  ...
  def __call__(self, a: cute.Tensor, b: cute.Tensor, c: cute.Tensor):
    ...
    wgmma_acc_569_epi_smem_layout = shared.get_smem_layout_row_major(cutlass.BFloat16, 128, 64, 2)
    wgmma_acc_569_tiled_mma = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 256, False)
    ...
    c_tma_atom_1, c_tma_tensor_1 = shared.get_tma_epi_tensor_and_atom(c, wgmma_acc_569_epi_smem_layout, 128, 64)
    ...
    self.kernel(a, b, st_571_layout, st_581_layout, c, wgmma_acc_569_epi_smem_layout, wgmma_acc_569_tiled_mma, tiled_mma_1011, tiled_mma_1032, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3).launch(grid=[132, 1, 1], block=384)

  @cute.kernel
  def kernel(self, a: cute.Tensor, b: cute.Tensor, st_571_layout, st_581_layout, c: cute.Tensor, wgmma_acc_569_epi_smem_layout, wgmma_acc_569_tiled_mma, tiled_mma_1011, tiled_mma_1032, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3):
    ...
    shared.smem_add_shared_tensor(SharedStorage_t, 'wgmma_acc_569_epi_smem_ptr', cutlass.BFloat16, wgmma_acc_569_epi_smem_layout, 1024)
    ...
    wgmma_acc_569_epi_smem = shared.smem_get_tensor(smem_, 'wgmma_acc_569_epi_smem_ptr', wgmma_acc_569_epi_smem_layout)
    ...
    if warpidx_ >= 0 and warpidx_ < 8:
      ...
      for sched_idx in cutlass.range(cute.arch.block_idx()[0], 192, 132):
        sched_coord = scheduler.remap_1d_idx(sched_idx, ((6, 32), (1, 1)), ((32, 1), (192, 32)), (32, 6), 6)
        wgmma_acc_569 = mma.get_acc(wgmma_acc_569_tiled_mma, 128, 256, cutlass.Float32)
        wgmma_acc_569_accumulate = False
        for k in cutlass.range(0, 112, 1):
          st_581_pipe.consumer_wait(st_581_cstate, st_581_pipe.consumer_try_wait(st_581_cstate))
          mma.accumulating_gemm_ss(tidx_, tiled_mma_1011, st_571, st_581, wgmma_acc_569, st_581_cstate, st_581_cstate, wgmma_acc_569_accumulate, -1)
          wgmma_acc_569_accumulate = True
          cute.nvgpu.warpgroup.wait_group(0)
          st_581_pipe.consumer_release(st_581_cstate)
          st_571_pstate.advance()
          st_581_cstate.advance()
        store.mma_epilogue_tma(tiled_mma_1032, c_tma_tensor_1, c_tma_atom_1, wgmma_acc_569_epi_smem, wgmma_acc_569, 128, 256, sched_coord[0], sched_coord[1], tidx_, warpidx_, cutlass.Float32)
        ...
        for sched_idx in cutlass.range(cute.arch.block_idx()[0], 192, 132):
          ...
          for k in cutlass.range(0, 112, 1):
            if cutlass.const_expr(True):
              st_581_pipe.producer_acquire(st_571_pstate, st_581_pipe.producer_try_acquire(st_571_pstate))
              mcast_mask_2, cta_coord_2, cta_layout_2 = shared.get_multicast_info(None, -1)
              shared.tma_copy(a_tma_atom_2, a_tma_tensor_2, st_571, 128, 64, sched_coord[0], k, st_581_pipe, st_571_pstate, cta_coord_2, cta_layout_2, mcast_mask_2)
              mcast_mask_3, cta_coord_3, cta_layout_3 = shared.get_multicast_info(None, -1)
              shared.tma_copy(b_tma_atom_3, b_tma_tensor_3, st_581, 256, 64, sched_coord[1], k, st_581_pipe, st_571_pstate, cta_coord_3, cta_layout_3, mcast_mask_3)
            st_571_pstate.advance()
