    ...
    if warpidx_ >= 0 and warpidx_ < 8:
      ...
      for sched_idx in cutlass.range(cute.arch.block_idx()[0], 128, 132):
        ...
        for k in cutlass.range(0, 64, 1):
          ...
          mma.accumulating_gemm_ss(tidx_, tiled_mma_7368, st_3259, st_3269, wgmma_acc_3233, st_3259_cstate, st_3259_cstate, wgmma_acc_3233_accumulate, -1)
          wgmma_acc_3233_accumulate = True
          cute.nvgpu.warpgroup.wait_group(0)
          st_3269_pipe.consumer_release(st_3259_cstate)
          st_3259_cstate.advance()
          st_3269_pstate.advance()
        st_3249_pipe.consumer_wait(st_3249_cstate, st_3249_pipe.consumer_try_wait(st_3249_cstate))
        rt_3245 = mma.copy_a_wgmma(tidx_, tiled_mma_7424, st_3235[None, None, st_3249_cstate.index], 128, 16, cutlass.BFloat16)
        mma.accumulating_gemm_rs(tidx_, tiled_mma_7473, rt_3245, st_3249, wgmma_acc_3233, st_3249_cstate, wgmma_acc_3233_accumulate, -1)
        wgmma_acc_3233_accumulate = True
        cute.nvgpu.warpgroup.wait_group(0)
        st_3249_pipe.consumer_release(st_3249_cstate)
        st_3249_pstate.advance()
        st_3249_cstate.advance()
        ...
        for sched_idx in cutlass.range(cute.arch.block_idx()[0], 128, 132):
          ...
          for k in cutlass.range(0, 64, 1):
            if cutlass.const_expr(True):
              st_3269_pipe.producer_acquire(st_3269_pstate, st_3269_pipe.producer_try_acquire(st_3269_pstate))
              mcast_mask_2, cta_coord_2, cta_layout_2 = shared.get_multicast_info(None, -1)
              shared.tma_copy(a_tma_atom_2, a_tma_tensor_2, st_3259, 128, 64, sched_coord[0], k, st_3269_pipe, st_3269_pstate, cta_coord_2, cta_layout_2, mcast_mask_2)
              mcast_mask_3, cta_coord_3, cta_layout_3 = shared.get_multicast_info(None, -1)
              shared.tma_copy(b_tma_atom_3, b_tma_tensor_3, st_3269, 256, 64, sched_coord[1], k, st_3269_pipe, st_3269_pstate, cta_coord_3, cta_layout_3, mcast_mask_3)
            st_3269_pstate.advance()
          if cutlass.const_expr(True):
            st_3249_pipe.producer_acquire(st_3249_pstate, st_3249_pipe.producer_try_acquire(st_3249_pstate))
            mcast_mask_4, cta_coord_4, cta_layout_4 = shared.get_multicast_info(None, -1)
            shared.tma_copy(xa_tma_atom_4, xa_tma_tensor_4, st_3235, 128, 16, sched_coord[0], 0, st_3249_pipe, st_3249_pstate, cta_coord_4, cta_layout_4, mcast_mask_4)
            mcast_mask_5, cta_coord_5, cta_layout_5 = shared.get_multicast_info(None, -1)
            shared.tma_copy(lora_b_tma_atom_5, lora_b_tma_tensor_5, st_3249, 256, 16, sched_coord[1], 0, st_3249_pipe, st_3249_pstate, cta_coord_5, cta_layout_5, mcast_mask_5)
          st_3249_pstate.advance()
