...
class Kernel:
  ...
  def __call__(self, a: cute.Tensor, b: cute.Tensor, xa: cute.Tensor, lora_b: cute.Tensor, c: cute.Tensor):
    ...
    wgmma_acc_6655_epi_smem_layout = shared.get_smem_layout_row_major(cutlass.BFloat16, 128, 64, 2)
    wgmma_acc_6655_tiled_mma = mma.get_tiled_mma(cutlass.BFloat16, True, True, cutlass.Float32, 128, 256, False)
    ...
    c_tma_atom_1, c_tma_tensor_1 = shared.get_tma_epi_tensor_and_atom(c, wgmma_acc_6655_epi_smem_layout, 128, 64)
  ...
  def kernel(self, a: cute.Tensor, b: cute.Tensor, xa: cute.Tensor, lora_b: cute.Tensor, st_6683_layout, st_6693_layout, st_6663_layout, st_6673_layout, c: cute.Tensor, wgmma_acc_6655_epi_smem_layout, wgmma_acc_6655_tiled_mma, tiled_mma_8448, tiled_mma_8547, tiled_mma_8568, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3, xa_tma_atom_4, xa_tma_tensor_4, lora_b_tma_atom_5, lora_b_tma_tensor_5):
    ...
    shared.smem_add_shared_tensor(SharedStorage_t, 'wgmma_acc_6655_epi_smem_ptr', cutlass.BFloat16, wgmma_acc_6655_epi_smem_layout, 1024)
    ...
    wgmma_acc_6655_epi_smem = shared.smem_get_tensor(smem_, 'wgmma_acc_6655_epi_smem_ptr', wgmma_acc_6655_epi_smem_layout)
    ...
    if warpidx_ >= 0 and warpidx_ < 8:
      ...
      for sched_idx in cutlass.range(cute.arch.block_idx()[0], 448, 132):
        sched_coord = scheduler.remap_1d_idx(sched_idx, ((8, 8), (7, 1)), ((8, 1), (64, 8)), (8, 56), 8)
        wgmma_acc_6655 = mma.get_acc(wgmma_acc_6655_tiled_mma, 128, 256, cutlass.Float32)
        wgmma_acc_6655_accumulate = False
        for k in cutlass.range(0, 64, 1):
          st_6693_pipe.consumer_wait(st_6693_cstate, st_6693_pipe.consumer_try_wait(st_6693_cstate))
          mma.accumulating_gemm_ss(tidx_, tiled_mma_8448, st_6683, st_6693, wgmma_acc_6655, st_6693_cstate, st_6693_cstate, wgmma_acc_6655_accumulate, -1)
          wgmma_acc_6655_accumulate = True
        ...
        mma.accumulating_gemm_ss(tidx_, tiled_mma_8547, st_6663, st_6673, wgmma_acc_6655, st_6663_cstate, st_6663_cstate, wgmma_acc_6655_accumulate, -1)
        wgmma_acc_6655_accumulate = True
        cute.nvgpu.warpgroup.wait_group(0)
        st_6673_pipe.consumer_release(st_6663_cstate)
        st_6663_pstate.advance()
        st_6663_cstate.advance()
        store.mma_epilogue_tma(tiled_mma_8568, c_tma_tensor_1, c_tma_atom_1, wgmma_acc_6655_epi_smem, wgmma_acc_6655, 128, 256, sched_coord[0], sched_coord[1], tidx_, warpidx_, cutlass.Float32)
      ...
      if warpidx_ == 8:
        ...
        for sched_idx in cutlass.range(cute.arch.block_idx()[0], 448, 132):
          ...
          for k in cutlass.range(0, 64, 1):
            ...
            st_6693_pstate.advance()
          if cutlass.const_expr(True):
            st_6673_pipe.producer_acquire(st_6663_pstate, st_6673_pipe.producer_try_acquire(st_6663_pstate))
            mcast_mask_4, cta_coord_4, cta_layout_4 = shared.get_multicast_info(None, -1)
            shared.tma_copy(xa_tma_atom_4, xa_tma_tensor_4, st_6663, 128, 16, sched_coord[0], 0, st_6673_pipe, st_6663_pstate, cta_coord_4, cta_layout_4, mcast_mask_4)
            mcast_mask_5, cta_coord_5, cta_layout_5 = shared.get_multicast_info(None, -1)
            shared.tma_copy(lora_b_tma_atom_5, lora_b_tma_tensor_5, st_6673, 256, 16, sched_coord[1], 0, st_6673_pipe, st_6663_pstate, cta_coord_5, cta_layout_5, mcast_mask_5)
          st_6663_pstate.advance()
