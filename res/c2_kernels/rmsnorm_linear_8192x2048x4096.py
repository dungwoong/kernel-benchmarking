  ...
  def kernel(self, a: cute.Tensor, b: cute.Tensor, st_1577_layout, st_1595_layout, c: cute.Tensor, rt_1642_epi_smem_layout, tiled_mma_1616, wgmma_acc_1606_tiled_mma, tiled_mma_5457, tiled_mma_5506, tiled_mma_5525, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3):
    ...
    if warpidx_ >= 0 and warpidx_ < 8:
      ...
      for sched_idx in cutlass.range(cute.arch.block_idx()[0], 512, 132):
        ...
        sched_coord = scheduler.remap_1d_idx(sched_idx, ((8, 8), (1, 8)), ((64, 1), (512, 8)), (64, 8), 8)
        ...
        for k in cutlass.range(0, 64, 1):
          ...
          mma.accumulating_gemm_rs(tidx_, tiled_mma_5506, rt_1587, st_1595, wgmma_acc_1606, st_1577_cstate, wgmma_acc_1606_accumulate, -1)
        ...
        for sched_idx in cutlass.range(cute.arch.block_idx()[0], 512, 132):
          ...
          for k in cutlass.range(0, 64, 1):
            if cutlass.const_expr(True):
              st_1595_pipe.producer_acquire(st_1595_pstate, st_1595_pipe.producer_try_acquire(st_1595_pstate))
              mcast_mask_2, cta_coord_2, cta_layout_2 = shared.get_multicast_info(None, -1)
              shared.tma_copy(a_tma_atom_2, a_tma_tensor_2, st_1577, 128, 64, sched_coord[0], k, st_1595_pipe, st_1595_pstate, cta_coord_2, cta_layout_2, mcast_mask_2)
              mcast_mask_3, cta_coord_3, cta_layout_3 = shared.get_multicast_info(None, -1)
              shared.tma_copy(b_tma_atom_3, b_tma_tensor_3, st_1595, 256, 64, sched_coord[1], k, st_1595_pipe, st_1595_pstate, cta_coord_3, cta_layout_3, mcast_mask_3)
            st_1595_pstate.advance()
