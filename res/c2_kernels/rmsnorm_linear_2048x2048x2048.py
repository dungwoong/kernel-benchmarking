    ...
    if warpidx_ >= 0 and warpidx_ < 8:
      ...
      for sched_idx in cutlass.range(cute.arch.block_idx()[0], 128, 132):
        ...
        for k in cutlass.range(0, 32, 1):
          st_1802_pipe.consumer_wait(st_1785_cstate, st_1802_pipe.consumer_try_wait(st_1785_cstate))
          rt_1795 = mma.copy_a_wgmma(tidx_, tiled_mma_5099, st_1785[None, None, st_1785_cstate.index], 128, 64, cutlass.BFloat16)
          mma.accumulating_gemm_rs(tidx_, tiled_mma_5148, rt_1795, st_1802, wgmma_acc_1812, st_1785_cstate, wgmma_acc_1812_accumulate, -1)
          wgmma_acc_1812_accumulate = True
          reduction.row_sum_square_mixed_types(rt_1795, rt_1821, cutlass.BFloat16)
          cute.nvgpu.warpgroup.wait_group(0)
          st_1802_pipe.consumer_release(st_1785_cstate)
          st_1785_cstate.advance()
          st_1802_pstate.advance()
        rt_1821.store(reduction.warp_sum_row_mma_layout(rt_1821.load()))
        rt_1832 = cute.make_rmem_tensor_like(rt_1821, cutlass.Float32)
        rt_1832.store(elementwise.const_div(rt_1821.load(), 2048.0))
        ...
        for sched_idx in cutlass.range(cute.arch.block_idx()[0], 128, 132):
          ...
          for k in cutlass.range(0, 32, 1):
            if cutlass.const_expr(True):
              st_1802_pipe.producer_acquire(st_1802_pstate, st_1802_pipe.producer_try_acquire(st_1802_pstate))
              mcast_mask_2, cta_coord_2, cta_layout_2 = shared.get_multicast_info(None, -1)
              shared.tma_copy(a_tma_atom_2, a_tma_tensor_2, st_1785, 128, 64, sched_coord[0], k, st_1802_pipe, st_1802_pstate, cta_coord_2, cta_layout_2, mcast_mask_2)
              mcast_mask_3, cta_coord_3, cta_layout_3 = shared.get_multicast_info(None, -1)
              shared.tma_copy(b_tma_atom_3, b_tma_tensor_3, st_1802, 256, 64, sched_coord[1], k, st_1802_pipe, st_1802_pstate, cta_coord_3, cta_layout_3, mcast_mask_3)
            st_1802_pstate.advance()
