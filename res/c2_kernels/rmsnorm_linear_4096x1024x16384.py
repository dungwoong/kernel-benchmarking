  ...
  def kernel(self, a: cute.Tensor, b: cute.Tensor, st_4677_layout, st_4695_layout, c: cute.Tensor, rt_4742_epi_smem_layout, tiled_mma_4716, wgmma_acc_4706_tiled_mma, tiled_mma_6234, tiled_mma_6185, tiled_mma_6255, c_tma_atom_1, c_tma_tensor_1, a_tma_atom_2, a_tma_tensor_2, b_tma_atom_3, b_tma_tensor_3):
    ...
    if warpidx_ >= 0 and warpidx_ < 8:
      ...
      for sched_idx in cutlass.range(cute.arch.block_idx()[0], 128, 132):
        ...
        rt_4730.store(elementwise.const_div(rt_4715.load(), 16384.0))
        ...
        for sched_idx in cutlass.range(cute.arch.block_idx()[0], 128, 132):
          ...
          for k in cutlass.range(0, 256, 1):
            if cutlass.const_expr(True):
              st_4695_pipe.producer_acquire(st_4695_pstate, st_4695_pipe.producer_try_acquire(st_4695_pstate))
              mcast_mask_2, cta_coord_2, cta_layout_2 = shared.get_multicast_info(None, -1)
              shared.tma_copy(a_tma_atom_2, a_tma_tensor_2, st_4677, 128, 64, sched_coord[0], k, st_4695_pipe, st_4695_pstate, cta_coord_2, cta_layout_2, mcast_mask_2)
              mcast_mask_3, cta_coord_3, cta_layout_3 = shared.get_multicast_info(None, -1)
              shared.tma_copy(b_tma_atom_3, b_tma_tensor_3, st_4695, 256, 64, sched_coord[1], k, st_4695_pipe, st_4695_pstate, cta_coord_3, cta_layout_3, mcast_mask_3)
            st_4695_pstate.advance()
