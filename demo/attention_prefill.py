...
class HelionAttention(AttentionBaseline):
    """helion attention: q/k/v are (B, H, S, D)"""
    ...
    def __call__(self, q):
        ...
        # back to (B, S, H, D) so it lines up with the reference
        return o.transpose(1, 2)
...
if __name__ == "__main__":
    ...
    # Helion loads its tuned config from helion_utils/autotune_cache
    helion_q = q.transpose(1, 2)
    helion_K = full_K.unsqueeze(0).transpose(1, 2)
    helion_V = full_V.unsqueeze(0).transpose(1, 2)
    compiled_attn_helion = Attention.compile(helion_q, helion_K, helion_V)
